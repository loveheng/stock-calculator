...
- ✅ 已修（0d87177 重构时落地）：step 内引入「同日单向互斥」守卫——当日已产生卖出意图则跳过全部买入意图；且卖（偏离 ≥ +dev%）与买（回踩 < MA20 / ≤ MA60）的价格条件本就互斥。回归保障：`strategyGenerators.test.ts`「同日买卖互斥」不变量（6 策略 × 趋势/摆动/横盘三种行情，任一撮合日不同时含买卖订单）。
...
| P2 | 卖出记账（计手续费 + 回流水）；ma20 同根买卖抖动 | 中 | 中 | ✅ 已修（卖出扣 SELL_BUFFER + max-opportunity 盈利回流为可投权益已修；ma20 同根抖动以「同日单向互斥」守卫阻断——见 §4.2，另补 6 策略同日买卖互斥回归单测） |

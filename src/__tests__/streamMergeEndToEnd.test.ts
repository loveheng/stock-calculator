 ...
 *                卖出 200 股 @17.43（倒T开仓借出）→ 底仓扣减 200 → 800 股
 *                买入 300 股 @16.00（其中 200 回补归还底仓，超额 100 归并）→ 1100 股 + 归并批次
 *              多轮场景：连续两轮倒T后底仓应为 1200 股（每轮净 +100）。
 * @layer Test
...
describe('addStreamRecord 倒T超额买回归并到底仓（端到端）', () => {
  ...
  test('单轮：卖出 200 + 买入 300 → 回补 200 归还底仓 + 归并 100 = 1100，且存在归并批次', () => {
  ...
  test('多轮：两轮倒T后底仓 = 1000 - 200 + 300 - 200 + 300 = 1200 股', () => {
    useAppStore.setState({ positions: [basePosition()] });
    ...
    expect(afterRound1.currentAmount).toBe(1100);
    ...
    // 净借出 = 累计卖出 400 - 累计买回 300 = 100 → 底仓 1000 - 100 = 900
    expect(afterSell2.currentAmount).toBe(900);
    ...
    const p = useAppStore.getState().positions[0];
    // 两轮均 CLEARED：买回覆盖全部卖出（净借出 0），每轮超额 100 归并底仓 → 1000 + 100 + 100 = 1200
    expect(p.currentAmount).toBe(1200);
  });
});

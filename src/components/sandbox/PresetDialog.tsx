...
export default function PresetDialog({ open, baseline, preset, onClose }: PresetDialogProps) {
  ...
  const [selected, setSelected] = useState<PresetStrategyId[]>(['ma20-bounce', 'pyramid', 'grid', 'stop-profit', 'gap-fill']);
  ...
  useEffect(() => {
    if (open) {
      if (preset && preset.presetStrategyId) {
        ...
        setSelected(
          (['ma20-bounce', 'pyramid', 'grid', 'stop-profit', 'gap-fill'] as PresetStrategyId[]).filter(
            (id) => !existingGenerated.includes(id),
          ),
        );
        setSimulatedCash(baseline ? String(Math.round(baseline.peakCapitalLock)) : '');
  ...
  const defaultCash = baseline?.peakCapitalLock ?? 0;

  const toggle = (id: PresetStrategyId) => {
    if (isUpdate) return;
    if (existingGenerated.includes(id)) return; // 已生成的策略不可再次生成
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div>
            ...
            />
            <p className="text-[10px] text-slate-600 mt-0.5">历史最高占用资金（可不改，调高可测"如果当初资金更多"）</p>
          </div>
          <div>
        ...
        <div className="space-y-2 overflow-y-auto flex-1 pr-1">
          {strategyList.filter((g) => (isUpdate ? g.id === lockedStrategyId : true)).map((g) => {
            const checked = selected.includes(g.id) || existingGenerated.includes(g.id);
                ...
                <div className="flex items-center gap-2 px-3 py-2.5 cursor-pointer" onClick={() => toggle(g.id)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-slate-200 font-medium">
                      {g.name}
                      {already && <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-slate-600/40 text-slate-300">已生成（唯一）</span>}
                    </div>
